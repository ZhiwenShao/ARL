...
__global__ void SigmoidCrossEntropyLossForwardGPU(const int nthreads, const Dtype* weights_, bool has_weights_, 
  ...
  CUDA_KERNEL_LOOP(i, nthreads) {
    ...
    if (has_ignore_label_ && target_value == ignore_label_) {
      ...
      Dtype temp_loss = input_data[i] * (target[i] - (input_data[i] >= 0)) -
          log(1 + exp(input_data[i] - 2 * input_data[i] *
          (input_data[i] >= 0)));
  		if (has_weights_) {
  			temp_loss *= weights_[i];
  		}
		  loss[i] = temp_loss;    
...
template <typename Dtype>
__global__ void SigmoidCrossEntropyLossIgnoreDiffGPU(const int count, const Dtype* weights_, bool has_weights_, 
...
void SigmoidCrossEntropyLossLayer<Dtype>::Forward_gpu(
  ...
  SigmoidCrossEntropyLossForwardGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
      CAFFE_CUDA_NUM_THREADS>>>(count, weights_, has_weights_, input_data, target, loss_data,
  ...
  if (propagate_down[0]) {
    ...
    if (has_ignore_label_) {
      ...
      SigmoidCrossEntropyLossIgnoreDiffGPU<Dtype><<<CAFFE_GET_BLOCKS(count),
        CAFFE_CUDA_NUM_THREADS>>>(count, weights_, has_weights_, ignore_label_, target, bottom_diff);
    }
    // Scale down gradient
