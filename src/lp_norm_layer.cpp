...
void L2NormLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  for (int i=0; i<num; ++i) {
    ...
    Dtype temp;
    caffe_cpu_scale(dim, inv_norm[i], top[0]->cpu_diff() + top[0]->offset(i), top_diff);
    temp = caffe_cpu_dot(dim, top_data, top[0]->cpu_diff() + top[0]->offset(i));
    caffe_cpu_scale(dim, -temp, top_data, bottom_diff);
    caffe_add(dim, bottom[0]->cpu_diff() + bottom[0]->offset(i), top[0]->cpu_diff() + top[0]->offset(i), bottom_diff);
  }
}
