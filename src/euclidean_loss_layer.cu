...
template <typename Dtype>
void EuclideanLoss6Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
	const vector<Blob<Dtype>*>& top) {
	int count = bottom[0]->count();
	true_count_ = 0;
	caffe_gpu_sub(
...
template <typename Dtype>
void EuclideanLoss6Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
	const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	for (int i = 0; i < 2; ++i) {
		if (propagate_down[i]) {
...
INSTANTIATE_LAYER_GPU_FUNCS(EuclideanLoss6Layer);
