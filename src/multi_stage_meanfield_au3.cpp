...
#include "caffe/layers/multi_stage_meanfield_au3.hpp"
...
void MultiStageMeanfieldAU3Layer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {

  const caffe::MultiStageMeanfieldAU3Parameter meanfield_param = this->layer_param_.multi_stage_meanfield_au3_param();
  ...
  feature_dim_ = bottom[2]->channels();
  ...
  bilateral_kernel_buffer_.reset(new float[(2+feature_dim_) * num_pixels_]);
  ...
  LOG(INFO) << ("MultiStageMeanfieldAU3Layer initialized.");
}

template <typename Dtype>
void MultiStageMeanfieldAU3Layer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
	// Do nothing.
}
 ...
 */
template <typename Dtype>
void MultiStageMeanfieldAU3Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top) {
  ...
  for (int n = 0; n < num_; ++n) {
    ...
    compute_bilateral_kernel(bottom[2], feature_dim_, n, bilateral_kernel_buffer_.get());
    bilateral_lattices_[n].reset(new ModifiedPermutohedral());
    bilateral_lattices_[n]->init(bilateral_kernel_buffer_.get(), 2+feature_dim_, num_pixels_);
 ...
 */
template<typename Dtype>
void MultiStageMeanfieldAU3Layer<Dtype>::Backward_cpu(
...
template<typename Dtype>
void MultiStageMeanfieldAU3Layer<Dtype>::compute_bilateral_kernel(const Blob<Dtype>* const feature_blob, const int feature_dim, const int n,
                                                               float* const output_kernel) {

  for (int p = 0; p < num_pixels_; ++p) {
    output_kernel[(2+feature_dim) * p] = static_cast<float>(p % width_) / theta_alpha_;
    output_kernel[(2+feature_dim) * p + 1] = static_cast<float>(p / width_) / theta_alpha_;

    const Dtype * const feature_data_start = feature_blob->cpu_data() + feature_blob->offset(n);
    for (int d = 0; d < feature_dim; ++d) {
    	output_kernel[(2+feature_dim)*p + 2 + d] = static_cast<float>((feature_data_start + num_pixels_*d)[p])/ theta_beta_;			
    }
  }
}

template <typename Dtype>
void MultiStageMeanfieldAU3Layer<Dtype>::compute_spatial_kernel(float* const output_kernel) {
...
INSTANTIATE_CLASS(MultiStageMeanfieldAU3Layer);
REGISTER_LAYER_CLASS(MultiStageMeanfieldAU3);
}  // namespace caffe
